...
namespace Math
{
    ...
    // Estrin poly for atan(z) on [0,1]
    static inline float atan_01_poly_estrin(float z) {
        // coefficients (single-precision)
        constexpr float c1 = 0.9998660f;
        constexpr float c3 = -0.3302995f;
        constexpr float c5 = 0.1801410f;
        constexpr float c7 = -0.0851330f;
        constexpr float c9 = 0.0208351f;

        const float z2 = z * z;
        const float z4 = z2 * z2;

        // Estrin, FMA-friendly: ((c9*z2 + c7)*z4) + ((c5*z2 + c3)*z2) + c1
        const float t1 = std::fmaf(c9, z2, c7);
        const float t2 = std::fmaf(c5, z2, c3);
        const float p = std::fmaf(t1, z4, std::fmaf(t2, z2, c1));
        return z * p;
    }

    // branchless copysign (faster than std::copysign on many compilers)
    static inline float copysignf_fast(float a, float y) {
        uint32_t ai = std::bit_cast<uint32_t>(a);
        const uint32_t yi = std::bit_cast<uint32_t>(y);
        ai = (ai & 0x7FFF'FFFFu) | (yi & 0x8000'0000u);
        return std::bit_cast<float>(ai);
    }

    // Vector-aware atan2: angle of (x,y) packed in v (x = lane0, y = lane1).
    // ~1e-5 rad max error in practice, branch-light.
    template<class T>
    static inline float atan2f_vec(const simd2::v2<T>& v)
    {
        const float x = (float)v.x();
        const float y = (float)v.y();

        // handle exact zero vector
        if (x == 0.0f && y == 0.0f)
            return std::numeric_limits<float>::quiet_NaN();

        if (x == 0.0f)
            return copysignf_fast(0.5f * float(pi_v<float>), y);

        const float ax = std::fabs(x);
        const float ay = std::fabs(y);

        // reduce to [0,pi/4]: z = min(ax,ay)/max(ax,ay)
        const float mn = (ax < ay) ? ax : ay;
        const float mx = (ax > ay) ? ax : ay;
        const float z = mn / mx;

        float a = atan_01_poly_estrin(z);

        // if |y| > |x| → a = pi/2 - a
        if (ay > ax) a = 0.5f * float(pi_v<float>) - a;

        // apply sign of y
        a = copysignf_fast(a, y);

        // if x < 0: reflect across ±pi
        if (x < 0.0f)
            a = (y >= 0.0f) ? (float(pi_v<float>) - a) : (-float(pi_v<float>) - a);
